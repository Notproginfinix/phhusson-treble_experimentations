...
\*** The Chinese version doesn't have NFC. If you want to stop hwmanager from flooring log with (try to start init): 
...
- Open any file explorer and go to `/odm/etc/vintf/` and Remove `manifest_nfc.xml`.
- Then go to `/odm/etc/permissions/sku_nfc/` and remove all files start with `android.hardware.nfc`.
...
2. You can re-lock your bootloader with stock ROM to restore your fingerprint. Backup your persist partition.
